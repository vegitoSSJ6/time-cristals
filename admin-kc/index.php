<?
  ...
  if($_GET['user']!=''){$postac=mysql_fetch_assoc(mysql_query("SELECT*FROM istoty WHERE user='".$_GET['user']."';"));
			if($postac["haslo"]==$_GET["haslo"]){include("pliki/system/zalogowany.php");}
			else{$tytul="Strona G³ówna"; naglowek($tytul); log_error(); login();};}
  ...
  else{$tytul="Strona G³ówna"; naglowek($tytul); login();};
  stopka();
?>
